<?php
    ...
    $recompensa = false; 
    if ($stmt = $conn->prepare("SELECT * FROM Recompensa WHERE activo='1';")) {
        $stmt->execute();
...
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css"> <style>
        ...
        .catalog-table, .reward-table {
            width: 100%;
            border-collapse: collapse;
            margin: 0;
        }

        .catalog-table thead th, .reward-table thead th {
            background-color: #007bff;
            color: white;
            padding: 12px 15px;
            text-align: left;
        ...
        .catalog-table tbody td, .reward-table tbody td {
            padding: 10px 15px;
        ...
        .catalog-table tbody tr:last-child td, .reward-table tbody tr:last-child td {
            border-bottom: none;
        }

        .catalog-table tbody tr:nth-child(even), .reward-table tbody tr:nth-child(even) {
...
<body>
    <div class="container">
        <div class="header">
            <i class="fas fa-box-open icon"></i> <h1>GESTIONAR CATÁLOGOS Y RECOMPENSAS</h1>
        </div>

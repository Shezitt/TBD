    ...
    <div class="container">
        ...
        <?php
            if (isset($_POST['reciclar'])) {
                ...
                $stmt = $conn->prepare("CALL sp_registrarReciclaje(?, ?, ?, ?, @p_puntos_ganados, @p_impacto_co2, @p_impacto_agua, @p_impacto_energia, @p_nivel_nuevo);");
                $stmt->bind_param("iiid", $idUsuario, $idMaterial, $idPunto, $cantidad);
                $estado = $stmt->execute();
                $stmt->close();

                if ($estado) {
                    $resultado = $conn->query("SELECT @p_puntos_ganados AS puntosGanados, @p_impacto_co2 AS impactoCO2, @p_impacto_agua AS impactoAgua, @p_impacto_energia AS impactoEnergia, @p_nivel_nuevo AS nivelNuevo");
                    $fila = $resultado->fetch_assoc();
                    if ($fila['nivelNuevo'] != $_SESSION['nivel']) {
                        $_SESSION['nivel'] = $fila['nivelNuevo'];
                        echo "<div class='message success'><h3>¡Felicidades!</h3>Has alcanzado un nuevo nivel: <strong>" . $fila['nivelNuevo'] . "</strong></div>";
                    }
